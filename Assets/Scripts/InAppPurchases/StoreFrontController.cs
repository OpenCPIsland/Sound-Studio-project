...
namespace InAppPurchases
{
	public class StoreFrontController : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			ScrollRectSnapTo obj = scrollRectSnapTo;
			obj.ElementSelected += new ScrollRectSnapTo.ElementSelectedDelegate(OnItemScrollSnap);
			scrollRectSnapTo.DisableScroll();
		}
		...
		public void SetupStoreItemViews(List<SkuInfo> skuList, SavedStorePurchasesCollection savedStorePurchasesCollection)
		{
			...
			StartCoroutine(ScrollingBuffer());
		}

		private IEnumerator ScrollingBuffer()
		{
			yield return null;
			scrollRectSnapTo.EnableScroll();
		}
