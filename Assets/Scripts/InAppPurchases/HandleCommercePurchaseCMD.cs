	...
	public class HandleCommercePurchaseCMD
	{
		...
		public void Execute()
		{
			...
			else if (storeType == StoreType.APPLE)
			{
				...
				mwsClient.RegisterApplePurchase(appId, Utils.GetGuid(), applePurchaseData, OnRegisterPurchaseComplete);
			...
			else if (storeType == StoreType.GOOGLE_PLAY)
			{
				...
				mwsClient.RegisterGooglePlayPurchase(appId, Utils.GetGuid(), googlePlayPurchaseData, OnRegisterPurchaseComplete);
				loadingOverlay.StartCoroutine(TimeoutforGetSkus());
			}
		}
