...
namespace UnityTest
{
    ...
    public class IntegrationTestsRunnerWindow : EditorWindow, IHasCustomMenu
    {
        ...
        private static void SelectInHierarchy(GameObject gameObject)
        {
            ...
            {
                var enable = test.GetComponentsInChildren(typeof(TestComponent), true)
                    .Any(c => c.gameObject == gameObject);
        ...
        private void RebuildTestList()
        {
            ...
            if (!EditorApplication.isPlayingOrWillChangePlaymode)
            {
                ...
                {
                    var existingTests = dynamicTestsOnScene.Where(c => c.dynamicTypeName == dynamicTestType.AssemblyQualifiedName);
            ...
            var oldDynamicResults = m_ResultList.Where(r => r.dynamicTest);
        ...
        private IntegrationTestRendererBase[] ParseTestList(List<TestComponent> testList, List<TestResult> results)
        {
            ...
            foreach (var testObject in testList)
            {
                ...
                var children = testObject.gameObject
                    .GetComponentsInChildren(typeof(TestComponent), true)
                    .Cast<TestComponent>()
                    .ToList();
